...
- 完整音乐库：账号歌单、收藏专辑、关注歌手、音乐云盘、最近播放（歌曲/歌单/专辑/播客）、播客电台发现（订阅、热门、分类）和听歌排行
...
- 歌词显示：可在仅原文、原文+译文、仅译文、仅罗马音之间切换，逐字高亮与背景人声行可独立开关，
  歌词时间轴支持 ±60 秒偏移（只改变歌词行的选取，不影响播放进度与 Seek）
...
npx @laoshuikaixue/cloudmusic-cli
```
...
npx @laoshuikaixue/cloudmusic-cli status --json
```
...
ncm search "A State Of Trance" --type radio
ncm play 186016
ncm pause
ncm resume
ncm seek +10
...
ncm library dj-categories
ncm library dj-category 3 --limit 20
ncm library dj-subscribed
ncm library dj-hot
...
- `/`：搜索网易云歌曲、歌单、专辑、歌手或电台；输入时按 `Tab` 切换搜索类型，电台搜索结果 Enter 进入节目列表后可直接播放
- `l`：打开音乐库，可浏览歌单、日推、歌单广场、官方榜单、新歌速递、播客发现、FM、心动模式、最近播放（歌曲/歌单/专辑/播客）、云盘、专辑、歌手和听歌排行
- 播客：音乐库“播客发现”可看订阅电台、热门电台和分类电台（分类接口返回的名称与 ID 原样展示）；从“最近播放 · 播客”或发现页进入任一电台可看到节目列表，节目按普通歌曲取源，`Enter` 播放、`a` 从第一期整队播放。播客队列不做听歌上报，避免把节目当成歌曲写进听歌记录
...
- `t`：循环切换歌词显示模式（仅原文 → 原文+译文 → 仅译文 → 仅罗马音）
- `[` / `]`：在播放页把歌词整体延后或提前 250 毫秒，可在“设置 → 歌词显示”里看到当前偏移
...
播放器会沿超清母带 → Hi-Res → 无损 → 极高 → 较高 → 标准逐级回退，播放页显示实际播放的音质。
高清环绕声、沉浸环绕声和杜比全景声依赖设备支持，不作为降级目标。
开启“失败跳过”后，自动切歌遇到没有音源的歌曲会继续向后尝试，连续 5 首仍失败时停止并给出原因
（如需要 VIP、需要购买、仅提供试听、暂无版权）；手动切歌始终如实返回失败原因。

播放倍速通过 mpv 的 `speed` 属性实时生效，范围 0.5 到 2.0，频谱会按倍速把播放位置换算回分析器
进度，因此倍速下频谱不会冻结或错位。响度归一（`off` / `track` / `album` 与 ±12 dB 前置增益）在
mpv 起播时下发，修改后从下一首开始生效。淡入淡出默认关闭，开启后在起播与恢复时淡入、暂停与切歌
前淡出，切歌前的淡出会占用等长的过渡时间。睡眠定时到点只暂停播放，daemon、队列与登录状态保持
在线；“播完当前曲”模式在当前歌曲结束时停止播放。
...
```bash
npx @laoshuikaixue/cloudmusic-cli mcp
```
...
提供的工具包括多类型搜索、播放控制、倍速与睡眠定时、队列、歌词、频谱快照、完整歌单、每日推荐、私人 FM、心动模式、
